...
--A query to see what book is due today (I'm using the date 04-13-17 for
--'Today'), and to see the name of whom the book is loaned out to.

SELECT lb.b_BranchName, bo.b_Title, bor.b_Name, bor.b_Address, bl.b_DateOut, bl.b_DueDate
FROM Book AS bo
...
--A query to see how many books are loaned out to each Library Branch,
...
--A query to retrieve the number of copies of every book authored by Stephen
